...
def graphData(graphName, dataList):
    ...
    plt.show()
    '''
    #graphData("soil moist raw", soilRawList)
    #multiple lists in one graph

    plt.plot(temperatureList, label="temp")
    plt.plot(humidityList, label="Hum")
    plt.plot(soilMoistureList,label="soilMoist")
    plt.plot(soilRawList, label="soilRaw")
    plt.legend()
    plt.show()
